...
/*
//module is any file which contains an import or export for TypeScript
export {}
...
describe('js array', () => {
    ...
    describe('filter duplicates in array', () => {
        ...
        it('using Set', async () => {
            const array = [1, 2, 3, 4, 4, 3, 5];
            const filteredArray = [...new Set<number>(array)];
            console.log(filteredArray);
            expect(filteredArray).to.have.length(5);
        });
    });
    ...
    describe('convert array to Map', () => {
        it('using reduce function', async () => {
            ...
            const resultMap = array.reduce((map, {name, type}) => map.set(name, type), new Map<string, string>());
